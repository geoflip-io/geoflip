...
from app.api.v1.operations.geoprocessing.transformation_manager import (
...
def transform_operation(
    ...
    try:
        ...
        if input_file_path:
            ...
            logger.info(
                f"Task {self.request.id}: Data read successfully {input_gdf.head()}"
            )
        else:
            raise ValueError(
                "Either input_file_path or data must be provided - not both."
            )

        # Apply transformations
        self.update_state(
            state="PROCESSING", meta={"message": "Applying transformations"}
        )
        input_gdf, transformations_applied = apply_transformations(
            gdf=input_gdf, transformations=transformations
        )
        logger.info(
            f"Task {self.request.id}: Transformations applied: {transformations_applied}"
        )
        ...
        if input_gdf is not None:
            output_type, output = gdf_to_output(
                input_gdf, output_format, output_epsg, job_id, to_file
            )
        else:
            logger.warning(
                f"Task {self.request.id}: input_gdf is None, no data to write."
            )
